...
Review the entire UniSearch project and add only these two interactions across the existing pages:

1. A back-to-top button that appears when the user scrolls down to 60% of the page height and scrolls smoothly to the top when clicked.
2. A subtle WhatsApp floating button that stays hidden by default, appears only while the user is actively scrolling, and disappears automatically after 3 seconds of no scrolling.

Important constraints:

- Do not change any unrelated styling, layout, logic, or existing functionality.
- Maintain the current design and behavior everywhere else.
- Keep the WhatsApp button low-visibility and not distracting.
- Check all pages that already contain the WhatsApp float element and apply the same behavior consistently.
- Ensure no JS or CSS errors are introduced and the site remains stable.
